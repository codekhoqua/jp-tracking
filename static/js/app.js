        ...
        if (card.dataset.tpKey === tpKey) {
            ...
            if (currentStatusClass !== `status-${newStatusClass}`) {
                ...
                if (tab) {
                    ...
                    // Update column badges and groups
...
function triggerCelebration() {
    ...
    if (typeof confetti === 'function') {
        ...
        (function frame() {
            confetti({
                particleCount: 4, angle: 60, spread: 55, origin: { x: 0.05, y: 0.6 }, zIndex: 9999,
                colors: ['#26ccff', '#a25afd', '#ff5e7e', '#88ff5a', '#fcff42', '#ffa62d', '#ff36ff'],
                disableForReducedMotion: true
            });
            confetti({
                particleCount: 4, angle: 120, spread: 55, origin: { x: 0.95, y: 0.6 }, zIndex: 9999,
                colors: ['#26ccff', '#a25afd', '#ff5e7e', '#88ff5a', '#fcff42', '#ffa62d', '#ff36ff'],
                disableForReducedMotion: true
            });
...
function showSuccessModal(customTitle, customBody) {
    triggerCelebration();

    let overlay = document.getElementById('success-modal-overlay');
    if (overlay) {
        overlay.remove(); // Remove existing to recreate with new text if needed
    }
    ...
    const titleText = customTitle || (isJa ? 'お疲れ様でした！' : 'お疲れ様でした！');
    const bodyText = customBody || (isJa ? '進捗が正常に記録されました。' : 'Logtime của bạn đã được ghi nhận thành công.');
    ...
    // Slight delay before opening to ensure DOM is ready
    setTimeout(() => {
...
function handleLogtime(event, formId) {
    ...
    fetch('/api/logtime', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
        ...
        .then(result => {
            ...
            if (progressLabel) progressLabel.textContent = result.status === 'success' ? '✅ Hoàn tất!' : '❌ Lỗi!';
...
function startTracker(index) {
    ...
    document.getElementById(`btn_start_${index}`).disabled = true;
    document.getElementById(`btn_end_${index}`).disabled = false;
...
function endTracker(index) {
    ...
    document.getElementById(`btn_start_${index}`).disabled = false;
    document.getElementById(`btn_end_${index}`).disabled = true;
    ...
    if (cb && cb.checked) {
        ...
        if(manualControls) {
            ...
            if (typeof flatpickr !== 'undefined') {
                const now = new Date();
                if (!document.getElementById(`manual_start_${index}`)._flatpickr) {
                ...
                if (!document.getElementById(`manual_end_${index}`)._flatpickr) {
...
function calcManualTime(index, field) {
    ...
    if (field === 'dur' || field === 'start') {
        if (durEl.value && parseFloat(durEl.value) > 0) {
            ...
            if (durEl.value) {
                ...
                durEl.value = parseFloat(diffHours.toFixed(2));
            }
        }
    }
...
async function saveTracker(index, tpKey) {
    ...
    if (isManual) {
        ...
        if (dur) {
            ...
            let sD = new Date(st.replace(' ', 'T'));
            let eD = new Date(et.replace(' ', 'T'));
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            if (!isManual) {
                ...
                localStorage.removeItem(`tracker_end_${index}`);
                document.getElementById(`time_display_${index}`).textContent = '00:00:00';
                document.getElementById(`btn_start_${index}`).disabled = false;
                document.getElementById(`btn_end_${index}`).disabled = true;
                updateCardBadge(index, false);
            } else {
                document.getElementById(`manual_start_${index}`).value = '';
                document.getElementById(`manual_end_${index}`).value = '';
                document.getElementById(`manual_duration_${index}`).value = '';
                document.getElementById(`manual_end_${index}`).disabled = false;
            }
            document.getElementById(`tracker_note_${index}`).value = '';
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.time-tracker-box').forEach(box => {
        ...
        if (startTime && !endTime) {
            ...
            let m = Math.floor((diff % 3600000) / 60000).toString().padStart(2, '0');
            let s = Math.floor((diff % 60000) / 1000).toString().padStart(2, '0');
            document.getElementById(`time_display_${index}`).textContent = `${h}:${m}:${s}`;
        }
    });
});
...
const tooltipFixPlugin = {
    id: 'tooltipFix',
    afterEvent(chart, args) {
        const evt = args.event;
        if (evt.type !== 'click' && evt.type !== 'touchstart') return;
        const nativeEvt = evt.native;
        if (!nativeEvt) return;

        const rect = chart.canvas.getBoundingClientRect();
        let cssX;
        if (nativeEvt.touches && nativeEvt.touches.length > 0) {
            cssX = nativeEvt.touches[0].clientX - rect.left;
        } else {
            cssX = nativeEvt.clientX - rect.left;
        }

        const mappedX = (cssX / rect.width) * chart.width;

        // Find first visible dataset to use as position reference
        let refMeta = null;
        for (let i = 0; i < chart.data.datasets.length; i++) {
            const meta = chart.getDatasetMeta(i);
        ...
        // Compare click mapped position with each element's rendered position
        let nearestIdx = 0;
        let minDist = Infinity;
        refMeta.data.forEach((el, idx) => {
            const dist = Math.abs(el.x - mappedX);
            if (dist < minDist) {
                minDist = dist;
                nearestIdx = idx;
            }
        ...
        // Build tooltip items for all visible datasets at this index
        const tooltipItems = [];
        chart.data.datasets.forEach((ds, dsIdx) => {
            const meta = chart.getDatasetMeta(dsIdx);
            if (!meta.hidden && meta.data[nearestIdx]) {
                tooltipItems.push({ datasetIndex: dsIdx, index: nearestIdx });
            }
        });

        if (tooltipItems.length > 0) {
            const posEl = refMeta.data[nearestIdx];
            chart.tooltip.setActiveElements(tooltipItems, { x: posEl.x, y: posEl.y });
            chart.setActiveElements(tooltipItems);
            args.changed = true;
        }
    }
};
...
function updateAreaChart(period) {
    ...
    if (customLegend) {
        ...
        if (legendOther) {
            legendOther.textContent = typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'vi' ? 'Prep' : 'Prep';
        }
    }
    ...
    if (period === 'week') {
        ...
        const getWorkerRole = (wName) => {
            if (typeof userProfilesDB !== 'undefined' && userProfilesDB) {
                const wLower = wName.toLowerCase();
                for (const [k, v] of Object.entries(userProfilesDB)) {
                    const kLower = k.toLowerCase();
                    if (kLower.includes(wLower) || wLower.includes(kLower)) {
                        return (v.team || '').toLowerCase();
                    }
                }
            }
            return '';
        ...
        labels.sort((a, b) => {
            if (a === txtOtherWorker) return 1;
            if (b === txtOtherWorker) return -1;
            const roleA = getWorkerRole(a);
            const roleB = getWorkerRole(b);
            const wA = roleA.includes('full') ? 1 : (roleA.includes('part') ? 2 : 3);
            const wB = roleB.includes('full') ? 1 : (roleB.includes('part') ? 2 : 3);
        ...
        let dataLettering = [];
        let dataCorrection = [];
        let dataLetteringRetouch = [];
        let dataOther = [];
        let detailRetouch = [];
        let detailLettering = [];
        let detailCorrection = [];
        let detailLetteringRetouch = [];
        let detailOther = [];
        ...
        labels.forEach(w => {
            ...
            let cTasks = tasks.filter(t => t.jobType === 'Lettering QC');
            let lrTasks = tasks.filter(t => t.jobType === 'Lettering/Retouch');
            let oTasks = tasks.filter(t => t.jobType !== 'Retouch' && t.jobType !== 'Lettering' && t.jobType !== 'Lettering QC' && t.jobType !== 'Lettering/Retouch');

            dataRetouch.push(rTasks.reduce((sum, t) => sum + t.weight, 0));
            dataLettering.push(lTasks.reduce((sum, t) => sum + t.weight, 0));
            dataCorrection.push(cTasks.reduce((sum, t) => sum + t.weight, 0));
            dataLetteringRetouch.push(lrTasks.reduce((sum, t) => sum + t.weight, 0));
            dataOther.push(oTasks.reduce((sum, t) => sum + t.weight, 0));

            detailRetouch.push(rTasks);
            detailLettering.push(lTasks);
            detailCorrection.push(cTasks);
            detailLetteringRetouch.push(lrTasks);
            detailOther.push(oTasks);
        });

        const getBorderRadius = (context) => {
            if (!context.chart || context.dataIndex === undefined) return 0;
            let isTop = true;
            ...
            for (let i = datasetIndex + 1; i < chart.data.datasets.length; i++) {
                ...
                const meta = chart.getDatasetMeta(i);
                if (!meta.hidden && ds.data[index] > 0) {
                    isTop = false;
                    break;
                }
            }
            return isTop ? {topLeft: 20, topRight: 20, bottomLeft: 0, bottomRight: 0} : 0;
        };
        ...
        chartDatasets = [
            ...
            },
            {
                label: 'Lettering QC',
                data: dataCorrection,
                backgroundColor: 'rgba(252, 165, 165, 0.85)', // Light Red
    ...
    taskChartInstance = new Chart(ctx, {
        ...
        plugins: isBarChart ? [avatarPlugin, tooltipFixPlugin] : [tooltipFixPlugin],
        options: {
            animations: {
                y: {
                    from: (ctx) => {
                    ...
                    callbacks: {
                        ...
                        afterBody: function (context) {
                            if (isBarChart) {
                                ...
                                const txtOtherJob = 'Prep';
                                let jobCounts = {};
                                tasks.forEach(t => {
                                    let jt = t.jobType || 'Prep';
                                    if (jt === 'Prep') jt = txtOtherJob;
...
function toggleCalendarPopup(deadlineText) {
    ...
    // Fix for strings like "Aug-7 (Fri)" returning Invalid Date or weird years
    const match = deadlineText.match(/([a-zA-Z]+)-(\d+)/);
    if (match) {
        const currentYear = new Date().getFullYear();
        const parsed = new Date(`${match[1]} ${match[2]}, ${currentYear}`);
...
document.addEventListener('DOMContentLoaded', () => {
    if (window.socket) {
        // Realtime Checklist Sync via socket (instant)
        window.socket.on('checklist_updated', () => {
            backgroundSyncChecklist();
        });
            ...
            if (useRadioPolling) {
                ...
                radioApiPost('/api/radio/claim').then(response => {
                    if (response && response.success) {
                        ...
                        showToast(`${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'すでにDJがいます！' : 'Đã có người làm DJ rồi!'} (${djName} ${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'がホストしています）。彼らがDJを終了するのを待ってください。' : 'đang host). Vui lòng đợi họ tắt DJ.'}`, 'error');
...
function setInputStatus(id, state) {
    ...
    function formatDateVietnamese(date) {
        const days = (typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? ['日', '月', '火', '水', '木', '金', '土'] : ['Chủ Nhật', 'Thứ Hai', 'Thứ Ba', 'Thứ Tư', 'Thứ Năm', 'Thứ Sáu', 'Thứ Bảy'];
...
async function submitPreparePsd() {
    ...
    if (!path || !tap) {
        showToast((typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja' ? "すべての情報を入力してください" : "Vui lòng nhập đầy đủ thông tin"), "error");
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            showToast(data.error || (typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja' ? "エラーが発生しました" : "Có lỗi xảy ra"), "error");
        }
    } catch (e) {
        setInputStatus('psd-path', 'error');
        setInputStatus('psd-tap', 'error');
        showToast((typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja' ? "ネットワークエラー: " : "Lỗi mạng: ") + e.message, "error");
    }
}
...
async function submitComparePsd() {
    ...
    if (!path || !tap || !sourcePath) {
        showToast((typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja' ? "ルートパス、エピソード数、ダウンロードパスをすべて入力してください" : "Vui lòng nhập đầy đủ đường dẫn gốc, số tập và đường dẫn tải về"), "error");
        return;
    }
...
window.renderLocalPlaylist = function() {
    ...
    if (list.length === 0) {
        container.innerHTML = `<div style="font-size: 10px; color: rgba(255,255,255,0.5); text-align: center; padding: 4px;">${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? '保存された曲はありません。' : 'Chưa có bài hát nào được lưu.'}</div>`;
        return;
    }
    let html = '';
    if (list.length > 1) {
        html += `
            <div style="display: flex; justify-content: center; padding: 4px; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 4px;">
                <button onclick="addAllLocalToQueue()" style="background: rgba(99,102,241,0.5); border: none; color: white; padding: 4px 12px; border-radius: 4px; cursor: pointer; font-size: 10px; font-weight: 600; width: 100%; transition: background 0.2s;" onmouseover="this.style.background='rgba(99,102,241,0.8)'" onmouseout="this.style.background='rgba(99,102,241,0.5)'">
                    ${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'すべてキューに追加' : 'Thêm tất cả vào hàng chờ'}
                </button>
            </div>
        `;
    }
    list.forEach(item => {
        ...
        html += `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 4px 6px; background: rgba(255,255,255,0.05); border-radius: 4px; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="this.style.background='rgba(255,255,255,0.05)'">
                ...
                <button onclick="removeLocalPlaylist('${item.id}', event)" style="background: none; border: none; color: #ff6b6b; cursor: pointer; padding: 0 4px; display: flex; align-items: center;" title="${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? '削除' : 'Xóa'}">
...
window.toggleDNASuggestions = function() {
    ...
    if (container.style.display === 'none' || container.style.display === '') {
        ...
        container.style.display = 'flex';
        container.innerHTML = `<div style="font-size: 10px; color: rgba(255,255,255,0.7); text-align: center; padding: 10px;">${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? '音楽のDNAを分析中...' : 'Đang phân tích DNA âm nhạc...'}</div>`;
            ...
            if (data.results && data.results.length > 0) {
                let html = `
                    <div style="font-size: 10px; font-weight: 600; color: #818cf8; text-align: center; padding: 2px 0 6px 0; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 4px;">
                        ${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'おすすめ:' : 'Gợi ý từ:'} ${keyword.substring(0,30)}${keyword.length > 30 ? '...' : ''}
                    </div>
                `;
                ...
                container.innerHTML = `<div style="font-size: 10px; color: rgba(255,255,255,0.5); text-align: center; padding: 10px;">${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'おすすめは見つかりませんでした。' : 'Không tìm thấy gợi ý nào.'}</div>`;
            }
        })
        .catch(err => {
            console.error(err);
            const container = document.getElementById('radio-dna-suggestions');
            if (container) container.innerHTML = `<div style="font-size: 10px; color: #ff6b6b; text-align: center; padding: 10px;">${(typeof CURRENT_LANG !== 'undefined' && CURRENT_LANG === 'ja') ? 'おすすめの読み込みエラー。' : 'Lỗi khi tải gợi ý.'}</div>`;
        });
}
...
function updateKanbanCounts(tabPane) {
    const columns = tabPane.querySelectorAll('.kanban-column');
    columns.forEach(col => {
        // filter out cards that are display: none
...
function updateKanbanGroups(tab) {
    tab.querySelectorAll('.kanban-column').forEach(col => {
        ...
        const children = Array.from(container.children);
        const headers = children.filter(c => c.classList.contains('job-group-header'));
        const sortSelect = tab.querySelector('.kanban-sort');
